...
export default function HomePage() {
  ...
  const customersWithUnpaid = customers.filter(
    (c) => (c.totalBill - c.totalPaid) > 0 && c.phone
  );
  ...
  const handleBroadcastWA = async () => {
    ...
    setProgress({ current: 0, total: customersWithUnpaid.length });
    let sent = 0;
    let failed = 0;
    ...
    for (const customer of customersWithUnpaid) {
      try {
        ...
        if (unpaidBills.length > 0 && customer.phone) {
          ...
          // Delay 5 seconds between messages to prevent WhatsApp Ban
            ...
            {sendingWA ? (
              <div className="bg-white p-4 rounded-xl border border-neutral-200 space-y-3">
                <div className="flex justify-between text-sm text-neutral-600">
                   ...
                   <b>{Math.round((progress.current / progress.total) * 100)}%</b>
                </div>
                ...
                <p className="text-center text-xs text-neutral-400">
                  {progress.current} dari {progress.total} terkirim. Jangan tutup halaman ini.
                </p>
              </div>
            ) : (
              <button
                onClick={handleBroadcastWA}
                className="w-full py-3 px-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors"
              >
                ...
                {`Kirim Tagihan ke ${customersWithUnpaid.length} Pelanggan`}
              </button>
            )}
            {broadcastResult && (
